...
object Main extends App {
  ...
  lazy val BASE_URL = "https://www.crowdcube.com/companies/"
  ...
  def getCompanyURIs(fetchCount: Int) = {
    val page: HtmlPage = wc.getPage(BASE_URL)
  ...
  def scrapeCompany(companyUri: String): Company = {
    ...
    val html = wc.getPage(BASE_URL + companyUri).asInstanceOf[HtmlPage].asXml
    ...
    new Company(frmtContent(matches.group(1)), frmtContent(matches.group(2)),
      TEAM_MEMBERS_RGX.findAllIn(html).matchData
        .map(tm => new TeamMember(frmtContent(tm.group(1)), null, tm.group(2).toUpperCase.contains("CEO"))).toSet, "")
  }
